...
pub const Response = union(MessageType) {
    ...
    pub fn fromJson(allocator: std.mem.Allocator, json_str: []const u8) !Response {
        ...
        if (parsed.value != .object) return error.InvalidHalValue;

        const obj = parsed.value.object;
        const type_val = obj.get("type") orelse return error.InvalidMessageType;
        if (type_val != .string) return error.InvalidMessageType;
        ...
        return switch (msg_type) {
            .list_pins => blk: {
                ...
                for (pins_arr.array.items) |pin_node| {
                    ...
                    if (name != .string) return error.InvalidHalValue;
                    if (pin_type_str != .string) return error.InvalidHalValue;
                    if (dir_str != .string) return error.InvalidHalValue;
                    ...
                    const parsed_value = try parseHalValue(&val);
                    // Debug: log first few values
                    if (pin_infos.items.len < 5) {
                        std.log.err("DEBUG: pin {s} type={s} value={}", .{name.string, pin_type_str.string, parsed_value});
                    }
            ...
            .list_signals => blk: {
                ...
                for (signals_arr.array.items) |sig_node| {
                    ...
                    if (name != .string) return error.InvalidHalValue;
                    if (sig_type_str != .string) return error.InvalidHalValue;
                    if (writers_arr != .array) return error.InvalidHalValue;
                    if (readers_arr != .array) return error.InvalidHalValue;
            ...
            .list_params => blk: {
                ...
                for (params_arr.array.items) |param_node| {
                    ...
                    if (name != .string) return error.InvalidHalValue;
                    if (param_type_str != .string) return error.InvalidHalValue;
                    if (dir_str != .string) return error.InvalidHalValue;
            ...
            .set_pin => blk: {
                ...
                if (val != .bool) return error.InvalidHalValue;
            ...
            .error_response => blk: {
                ...
                if (val != .string) return error.InvalidHalValue;
        ...
        if (obj.get("float")) |v| {
            ...
            switch (v) {
                ...
                .integer => |i| return HalValue{ .float = @floatFromInt(i) },
                    ...
                    std.log.err("parseHalValue: invalid 'float' value type: {s}", .{@tagName(v)});

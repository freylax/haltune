...
/// Pin direction (from HAL)
pub const PinDir = enum {
    in,
    out,
    io,
    unspecified,
...
/// Get pin direction by parsing halcmd show output
/// Returns the pin direction or error.PinNotFound if pin doesn't exist
pub fn getPinDir(allocator: std.mem.Allocator, pin_name: []const u8) !PinDir {
    const cmd_result = try runHalcmd(allocator, &[_][]const u8{ "show", "pin", pin_name });
    defer cmd_result.deinit();

    // Parse output format:
    // Owner   Type  Dir         Value  Name
    //    11  bit   IN          FALSE  pid.enable

    var lines = std.mem.tokenizeScalar(u8, cmd_result.output, '\n');
    while (lines.next()) |line| {
        const trimmed = std.mem.trimLeft(u8, line, " ");
        // Skip header lines
        if (std.mem.startsWith(u8, trimmed, "Owner") or
            std.mem.startsWith(u8, trimmed, "Component")) {
            continue;
        }

        // Parse line: owner type dir value name
        var parts = std.mem.tokenizeScalar(u8, trimmed, ' ');
        var part_num: usize = 0;
        while (parts.next()) |part| {
            part_num += 1;
            // Direction is the 3rd column (after Owner and Type)
            if (part_num == 3) {
                if (std.mem.eql(u8, part, "IN")) {
                    return .in;
                } else if (std.mem.eql(u8, part, "OUT")) {
                    return .out;
                } else if (std.mem.eql(u8, part, "IO")) {
                    return .io;
                }
            }
        }
